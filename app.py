...
import tempfile
from flask import Flask, make_response, render_template, request, redirect, url_for, flash, jsonify, send_file, session
import os
import shutil
...
app.config['MAX_CONTENT_LENGTH'] = 200 * 1024 * 1024  # 16 MB max upload size
...
# ==========================================
# PYPSA VISUALIZATION: APP-LEVEL HELPERS
# ==========================================
TEMP_DIR = tempfile.mkdtemp(prefix="pypsa_flask_")

def get_pypsa_results_folder(app_instance):
    """Get the folder path for PyPSA results based on current project"""
    current_project_base_path = app_instance.config.get('CURRENT_PROJECT_PATH')
    if not current_project_base_path:
        logger.error("get_pypsa_results_folder: CURRENT_PROJECT_PATH not set in app.config.")
        return None
    pypsa_results_relative_path = os.path.join('results', 'Pypsa_results')
    results_folder_path = os.path.join(current_project_base_path, pypsa_results_relative_path)
    return results_folder_path

def _get_snapshots_for_period(n: pypsa.Network, 
                              period_name: Optional[str] = None, 
                              start_date_str: Optional[str] = None, 
                              end_date_str: Optional[str] = None) -> Union[pd.DatetimeIndex, pd.MultiIndex]:
    """Extract relevant snapshots based on period and/or date range."""
    ...
    # Filter by period if provided and applicable
    if period_name and isinstance(current_snapshots_slice, pd.MultiIndex):
        ...
        period_level_dtype = period_level_values.dtype
        
        try:
            # Convert period_name to appropriate type based on the index
            if pd.api.types.is_integer_dtype(period_level_dtype): 
                typed_period_name = int(period_name)
            elif pd.api.types.is_float_dtype(period_level_dtype): 
                typed_period_name = float(period_name)
            else: 
                typed_period_name = str(period_name)
            ...
            if period_mask.any(): 
                ...
                logger.warning(f"Period '{period_name}' not found.")
                
        except ValueError: 
            logger.warning(f"Could not convert period name '{period_name}'.")
        except Exception as e: 
            logger.error(f"Error period filtering: {e}", exc_info=True)
        ...
        try:
            ...
            if time_component is not None and not time_component.empty:
                ...
                if isinstance(current_snapshots_slice, pd.MultiIndex):
                    ...
                    current_snapshots_slice = current_snapshots_slice[
                        current_snapshots_slice.get_level_values(-1).isin(valid_time_indices)
                    ]
                else:
                    current_snapshots_slice = current_snapshots_slice[date_mask]
                    
                if current_snapshots_slice.empty:
                    logger.warning("Date range resulted in empty snapshots.")
        except Exception as e:
...
def _api_data_wrapper(network_rel_path, data_extraction_func_name_in_pau: str, **route_specific_kwargs):
    """Generic wrapper for PyPSA data extraction API endpoints."""
    pypsa_folder = get_pypsa_results_folder(app) # app is the Flask app instance
    if not pypsa_folder: 
        logger.error(f"_api_data_wrapper: Project context not available or results folder path could not be determined for {network_rel_path}, func {data_extraction_func_name_in_pau}")
        return make_response(jsonify({'status': 'error', 'message': 'Project context not available.'}), 500)
    ...
    if not full_path.startswith(os.path.normpath(pypsa_folder)): 
        logger.warning(f"_api_data_wrapper: Invalid file path access attempt: {network_rel_path} for func {data_extraction_func_name_in_pau}")
        return make_response(jsonify({'status': 'error', 'message': 'Invalid file path.'}), 400)
        
    if not os.path.exists(full_path):
        logger.error(f"_api_data_wrapper: Network file not found: {full_path} (from relative: {network_rel_path}) for func {data_extraction_func_name_in_pau}")
        return make_response(
            jsonify({'status': 'error', 'message': f'Network file not found: {network_rel_path}'}), 
            404
        )
    ...
    try:
        ...
        logger.info(f"_api_data_wrapper: Successfully imported network '{os.path.basename(full_path)}' for func '{data_extraction_func_name_in_pau}'. Components: {', '.join(n.components.keys()) if hasattr(n, 'components') else 'N/A'}")
        
        actual_data_func = getattr(pau, data_extraction_func_name_in_pau, None)
        if not callable(actual_data_func):
            logger.critical(f"_api_data_wrapper: Data extraction function '{data_extraction_func_name_in_pau}' not found in pypsa_analysis_utils.")
            return make_response(jsonify({'status': 'error', 'message': f'Server misconfiguration: Data function {data_extraction_func_name_in_pau} not found.'}), 500)

        filtered_snapshots = _get_snapshots_for_period(n, period_name_req, start_date_req, end_date_req)
        
        if filtered_snapshots.empty:
            logging.warning(
                f"_api_data_wrapper: No snapshots selected after filtering for func '{data_extraction_func_name_in_pau}' "
                f"(Network: {network_rel_path}, Period: {period_name_req}, Date: {start_date_req}-{end_date_req}, Res: {resolution_req}). "
                f"Returning empty success response."
            )
            empty_payload_content = {}
            # Define schema-correct empty data for known payload formers
            if data_extraction_func_name_in_pau == "dispatch_data_payload_former":
                empty_payload_content = {'generation': [], 'load': [], 'storage': [], 'store': [], 'timestamps': []}
                 ...
                 empty_payload_content = {'available': False, 'message': 'No data for selected filters.', 'avg_by_bus':[], 'duration_curve':[], 'timestamps':[], 'buses':[]}
            elif data_extraction_func_name_in_pau == "extract_api_network_flow_payload_former":
                 empty_payload_content = {'losses': [], 'line_loading': []}
                
            response_key_base = data_extraction_func_name_in_pau.replace("get_", "").replace("_payload", "").replace("_former","")
            response_dict = {
                'status': 'success',
        ...
        func_args = {
            ...
            'snapshots_slice': filtered_snapshots,
            'resolution': resolution_req,
            'period': period_name_req, 
            'period_name': period_name_req,
        }
        ...
        if '_n' in sig.parameters and 'n' not in valid_func_args and '_n' not in valid_func_args :
             valid_func_args['_n'] = n

        logger.debug(f"_api_data_wrapper: Calling {data_extraction_func_name_in_pau} with args: {list(valid_func_args.keys())}")
        ...
        # Ensure handle_nan_values is accessible
        nan_handler = getattr(current_app, 'handle_nan_values', handle_nan_values)
        ...
        def serialize_item_for_json(item):
            if isinstance(item, pd.DataFrame):
                ...
                if is_multi:
                    for i, orig_name in enumerate(original_index_names):
                        col_from_reset = df_reset.columns[i]
                        if col_from_reset != orig_name:
                            df_reset.rename(columns={col_from_reset: orig_name}, inplace=True)
                elif len(original_index_names) == 1 and original_index_names[0] != 'index' and df_reset.columns[0] == 'index':
                ...
                for col in df_reset.columns:
                    if pd.api.types.is_datetime64_any_dtype(df_reset[col]):
                        ...
                        df_reset[col] = df_reset[col].astype(str)
                return nan_handler(df_reset.to_dict(orient='records'))
                ...
                if series_reset.columns[0] != idx_name :
                    series_reset.rename(columns={series_reset.columns[0]: idx_name}, inplace=True)
                if pd.api.types.is_datetime64_any_dtype(series_reset[idx_name]):
                    series_reset[idx_name] = series_reset[idx_name].astype(str)
                return nan_handler(series_reset.to_dict(orient='records'))
            return nan_handler(item)
        ...
        response_dict = {
            'status': 'success',
            response_key_base + "_data": serialized_payload_content,
        }
        if color_palette:
             response_dict['colors'] = color_palette
        
        logger.info(f"_api_data_wrapper: Successfully processed func '{data_extraction_func_name_in_pau}' for network '{network_rel_path}'")
        return jsonify(response_dict)

    except FileNotFoundError: 
        logger.error(f"_api_data_wrapper: FileNotFoundError for {network_rel_path} within wrapper for func {data_extraction_func_name_in_pau}.", exc_info=True)
        return make_response(jsonify({'status': 'error', 'message': f'Network file disappeared or is inaccessible: {network_rel_path}'}), 404)
    except Exception as e:
        logger.error(
            f"_api_data_wrapper: CRITICAL ERROR during call to '{data_extraction_func_name_in_pau}' "
            f"for network '{network_rel_path}'. Filters: Period='{period_name_req}', "
            f"Start='{start_date_req}', End='{end_date_req}', Res='{resolution_req}'. Error: {type(e).__name__} - {str(e)}", 
            exc_info=True
        )
        error_payload = {
            'status': 'error', 
            'message': f"An error occurred on the server while processing data for '{data_extraction_func_name_in_pau}'.",
            'details': f"{type(e).__name__}: {str(e)}" # More detailed error for debugging
        }
...
# ==========================================
# PYPSA VISUALIZATION API ROUTES
# ==========================================

@app.route('/pypsa_results')
def pypsa_results():
    """Main route for the PyPSA results visualization page."""
    logger.info("Accessing PyPSA results page")
    if app.config['CURRENT_PROJECT_PATH'] is None: 
        flash('Please select or create a project first.', 'warning')
        return redirect(url_for('home'))
    
    pypsa_folder = get_pypsa_results_folder(app)
    scenarios = []
    nc_files_info = []
                ...
                for root, _, files in os.walk(item_path):
                    for file_item in files:
                        if file_item.endswith('.nc'):
                            ...
                            nc_files_info.append({
                                'scenario': item,
                                'filename': file_item,
                                'path': rel_path_to_file.replace(os.sep, '/'),
                            })
    else:
        if pypsa_folder: 
    ...
    return render_template('pypsa_results.html',
                           ...
                           current_project=app.config.get('CURRENT_PROJECT', 'N/A'))


@app.route('/api/pypsa/scan_files', methods=['GET'])
def api_scan_pypsa_files_route():
    """API route to scan for PyPSA network files."""
    pypsa_folder = get_pypsa_results_folder(app)
    if not pypsa_folder: 
        return jsonify({'status': 'error', 'message': 'No project selected.'})
...
@app.route('/api/pypsa/upload_network', methods=['POST'])
def api_upload_pypsa_network_route():
    """API route to upload a PyPSA network file."""
    pypsa_folder = get_pypsa_results_folder(app)
    if not pypsa_folder: 
        return jsonify({'status': 'error', 'message': 'No project selected.'})
...
@app.route('/api/pypsa/network_info/<path:network_rel_path>', methods=['GET'])
def api_get_pypsa_network_info_route(network_rel_path):
    """API route to get basic network information."""
    pypsa_folder = get_pypsa_results_folder(app)
    if not pypsa_folder: 
        return jsonify({'status': 'error', 'message': 'Project context not available.'})
    
    # Ensure network_rel_path is correctly joined and validated
    full_path = os.path.normpath(os.path.join(pypsa_folder, network_rel_path))
    if not full_path.startswith(os.path.normpath(pypsa_folder)):
        logger.error(f"Path traversal attempt detected: {network_rel_path}")
    ...
    try:
        ...
        # Get component counts - fixed to handle different PyPSA versions
        components_info = {}
        if hasattr(n, 'components'):
            # Handle PyPSA's components structure appropriately
            for component_name in n.components.keys():
                # Get the attribute name for this component type
                try:
                    ...
                    # Handle case where components structure is different
                    try:
                        # Try direct access if it's a DataFrame-like structure
        ...
        # Fallback if components structure is totally different
        if not components_info:
            logger.warning(f"Could not extract components from network {network_rel_path} using traditional method")
            # Try to infer components from attributes that look like DataFrames
            for attr_name in dir(n):
                if attr_name.startswith('_'):
                    continue
                try:
                    attr = getattr(n, attr_name)
                    if hasattr(attr, 'shape') and hasattr(attr, 'index'):
                        components_info[attr_name] = len(attr)
                except:
                    pass
        ...
        if hasattr(n, 'carriers'):
            try:
                carriers_list = n.carriers.index.tolist() if not n.carriers.empty else []
            except:
                # In case carriers is structured differently
                if hasattr(n.carriers, 'keys'):
                    carriers_list = list(n.carriers.keys())
        
        start_s, end_s = "N/A", "N/A"
        if not snaps.empty:
            s0, s_end = snaps[0], snaps[-1]
            # Handle tuple case for MultiIndex elements
            start_s = str(s0[-1] if isinstance(s0, tuple) else s0)
            end_s = str(s_end[-1] if isinstance(s_end, tuple) else s_end)
        ...
        logger.error(f"Error loading network info for {network_rel_path}: {e}", exc_info=True)
        error_payload = {'status': 'error', 'message': f"Error processing network info: {str(e)}"}
        response = make_response(jsonify(error_payload), 500)
        response.mimetype = "application/json"
        return response

@app.route('/api/pypsa/extract_period/<path:network_rel_path>/<period_name_req>', methods=['GET'])
def api_extract_period_network_route(network_rel_path, period_name_req):
    """API route to extract a specific period from a multi-period network."""
    pypsa_folder = get_pypsa_results_folder(app)
    if not pypsa_folder: 
        return jsonify({'status': 'error', 'message': 'No project'})
        
    original_full_path = os.path.join(pypsa_folder, network_rel_path)
    if not os.path.exists(original_full_path): 
        return jsonify({'status': 'error', 'message': 'Original network not found'})
        
    try:
        n_orig = pypsa.Network(original_full_path)
        snaps_orig = pau.safe_get_snapshots(n_orig)
        
        if not isinstance(snaps_orig, pd.MultiIndex) or snaps_orig.nlevels == 0: 
            return jsonify({'status': 'error', 'message': 'Not multi-period'})
        
        period_level_dtype = snaps_orig.levels[0].dtype
        try:
            period_val = int(period_name_req) if pd.api.types.is_integer_dtype(period_level_dtype) else \
                         float(period_name_req) if pd.api.types.is_float_dtype(period_level_dtype) else str(period_name_req)
        except ValueError: 
            return jsonify({'status': 'error', 'message': f'Invalid period format: {period_name_req}'})

        period_snaps = snaps_orig[snaps_orig.get_level_values(0) == period_val]
        if period_snaps.empty: 
            return jsonify({'status': 'error', 'message': f'Period {period_name_req} not found.'})
        
        n_period = pypsa.Network()
        
        # Extract static component data
        for comp_name, comp_list_name in n_orig.components['list_name'].items():
            df_orig = getattr(n_orig, comp_list_name)
            if not df_orig.empty:
                if "build_year" in df_orig.columns and "lifetime" in df_orig.columns:
                    active_mask = (df_orig["build_year"] <= period_val) & ((df_orig["build_year"] + df_orig["lifetime"]) > period_val)
                    setattr(n_period, comp_list_name, df_orig[active_mask].copy())
                else:
                    setattr(n_period, comp_list_name, df_orig.copy())
                    
        # Copy carriers data
        if hasattr(n_orig, 'carriers'):
            n_period.carriers = n_orig.carriers.copy()
            
        # Set snapshots to just the time component
        time_idx_period = period_snaps.get_level_values(-1)
        n_period.set_snapshots(time_idx_period)
        
        # Extract time-dependent component data
        for comp_t_name in n_orig.all_components_t:
            pnl_orig = getattr(n_orig, comp_t_name)
            pnl_period = {}
            
            for key, df_t_orig in pnl_orig.items():
                common_idx_t = df_t_orig.index.intersection(period_snaps)
                if not common_idx_t.empty:
                    df_t_period = df_t_orig.loc[common_idx_t]
                    df_t_period.index = pau.get_time_index(common_idx_t)
                    pnl_period[key] = df_t_period
                    
            if pnl_period:
                setattr(n_period, comp_t_name, pnl_period)
                
        # Copy snapshot weights if present
        if hasattr(n_orig, 'snapshot_weightings') and not n_orig.snapshot_weightings.empty:
            s_weights_orig = n_orig.snapshot_weightings
            common_weights_idx = s_weights_orig.index.intersection(period_snaps)
            
            if not common_weights_idx.empty:
                s_weights_period = s_weights_orig.loc[common_weights_idx]
                s_weights_period.index = pau.get_time_index(common_weights_idx)
                n_period.snapshot_weightings = s_weights_period

        # Save the extracted period network to file
        extracted_fname = f"{os.path.splitext(os.path.basename(network_rel_path))[0]}_period_{period_name_req}.nc"
        save_dir = os.path.join(os.path.dirname(original_full_path), "extracted_periods")
        os.makedirs(save_dir, exist_ok=True)
        
        save_path = os.path.join(save_dir, extracted_fname)
        n_period.export_to_netcdf(save_path)
        
        rel_save_path = os.path.relpath(save_path, pypsa_folder)
        return jsonify({
            'status': 'success', 
            'file_info': {
                'filename': extracted_fname, 
                'path': rel_save_path.replace(os.sep, '/')
            }
        })
    except Exception as e:
        logger.error(f"Extract period error: {e}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)})


# API routes using the wrapper and pau function names
@app.route('/api/pypsa/dispatch_data/<path:network_rel_path>', methods=['GET'])
def api_get_dispatch_data_route(network_rel_path):
    """API route to get generation dispatch data."""
    return _api_data_wrapper(network_rel_path, "dispatch_data_payload_former")

@app.route('/api/pypsa/capacity_data/<path:network_rel_path>', methods=['GET'])
def api_get_capacity_data_route(network_rel_path):
    """API route to get installed capacity data."""
    attribute = request.args.get('attribute', 'p_nom_opt')
    return _api_data_wrapper(network_rel_path, "get_carrier_capacity", attribute=attribute)

@app.route('/api/pypsa/metrics_data/<path:network_rel_path>', methods=['GET'])
def api_get_metrics_data_route(network_rel_path):
    """API route to get CUF and curtailment metrics."""
    return _api_data_wrapper(network_rel_path, "combined_metrics_extractor_wrapper")

@app.route('/api/pypsa/storage_data/<path:network_rel_path>', methods=['GET'])
def api_get_storage_data_route(network_rel_path):
    """API route to get storage operation data."""
    return _api_data_wrapper(network_rel_path, "extract_api_storage_data_payload_former")

@app.route('/api/pypsa/emissions_data/<path:network_rel_path>', methods=['GET'])
def api_get_emissions_data_route(network_rel_path):
    """API route to get CO2 emissions data."""
    return _api_data_wrapper(network_rel_path, "emissions_payload_former")

@app.route('/api/pypsa/prices_data/<path:network_rel_path>', methods=['GET'])
def api_get_prices_data_route(network_rel_path):
    """API route to get marginal price data."""
    return _api_data_wrapper(network_rel_path, "extract_api_prices_data_payload_former")

@app.route('/api/pypsa/network_flow/<path:network_rel_path>', methods=['GET'])
def api_get_network_flow_data_route(network_rel_path):
    """API route to get network flow and losses data."""
    return _api_data_wrapper(network_rel_path, "extract_api_network_flow_payload_former")

@app.route('/api/pypsa/compare_networks', methods=['POST'])
def api_compare_networks_route():
    """API route to compare multiple networks."""
    pypsa_folder = get_pypsa_results_folder(app)
    if not pypsa_folder: 
        return jsonify({'status': 'error', 'message': 'No project'})
    ...
    try:
        ...
        paths = req.get('file_paths', [])
        labels = req.get('labels', {})
        ctype = req.get('comparison_type', 'capacity')
        
        if len(paths) < 2:
            return jsonify({'status': 'error', 'message': 'Need at least 2 networks'})
        ...
        for rel_path in paths:
            full_path = os.path.join(pypsa_folder, rel_path)
            label = labels.get(rel_path, os.path.basename(rel_path))
            
            if not os.path.exists(full_path):
                continue
                
            n_comp = pypsa.Network(full_path)
            combined_colors.update(pau.get_color_palette(n_comp))
            
            if ctype == 'capacity':
                attr = req.get('attribute', 'p_nom_opt')
                unit = 'MWh' if 'e_nom' in attr else 'MW'
                df = pau.get_carrier_capacity(n_comp, attribute=attr)
                
                if df is not None and not df.empty and 'Market' in df.index:
                    df = df[df.index != 'Market']
                    
                all_data[label] = df.reset_index().to_dict('records') if not df.empty else []
                
            elif ctype == 'generation':
                unit = 'MWh'
                gen_d, _, _, _ = pau.get_dispatch_data(n_comp)
                
                if not gen_d.empty:
                    total_g = gen_d.sum()
                    all_data[label] = total_g.reset_index().rename(
                        columns={'index': 'Carrier', 0: 'Generation'}
                    ).to_dict('records')
                else:
                    all_data[label] = []
                ...
                all_data[label]['cuf'] = pau.calculate_cuf(n_comp).to_dict('records')
                all_data[label]['curtailment'] = pau.calculate_curtailment(n_comp).to_dict('records')
                
            elif ctype == 'emissions':
                unit = 'Tonnes CO2'
                total_e, e_by_c = pau.calculate_co2_emissions(n_comp)
                
                if label not in all_data:
                    all_data[label] = {}
                    
                all_data[label]['total'] = total_e.to_dict('records')
                all_data[label]['by_carrier'] = e_by_c.to_dict('records')
        
        payload_data = all_data
        if ctype == 'metrics':
            # Restructure metrics for easier JS consumption
            cuf_p, curt_p = {}, {}
            for net_label, metric_data_val in all_data.items():
                cuf_p[net_label] = metric_data_val.get('cuf', [])
                curt_p[net_label] = metric_data_val.get('curtailment', [])
            payload_data = {'cuf': cuf_p, 'curtailment': curt_p}
